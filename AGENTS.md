...
The repository packages a single containerised script, `when.py`. Keep the project aligned with this layout when adding or updating files.
...
2. Run dependency-free unit tests:
   ...
   pytest
   ```
3. Update `spec.md` whenever `when.py` or its CLI surface changes.
...
- Place tests in the `tests/` directory adjacent to `when.py` and mock external commands or filesystem interactions so unit tests exercise only the code under test.
