...
The repository packages a single containerised script, `fitdisk.py`. Keep the project aligned with this layout when adding or updating files.
...
2. Run dependency-free unit tests:
   ...
   pytest
   ```
3. Update `spec.md` whenever `fitdisk.py` or its CLI surface changes.
...
- Place tests in the `tests/` directory adjacent to `fitdisk.py` and mock external commands or filesystem interactions so unit tests exercise only the code under test.
