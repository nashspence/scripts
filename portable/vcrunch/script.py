...
def main():
    ...
    ap.add_argument(
        "--target-size", default="25G", help="Total target size (default 25G)."
    )
    ap.add_argument(
    ...
    if all_videos_done(manifest, args.output_dir):
        ...
        eprint(
            "Manifest indicates all videos are done and outputs are valid. Nothing to do."
        )
    ...
    eprint(
        f"Total inputs: {len(all_files)}  (videos: {len(videos)}, assets: {len(assets)})"
    )
    eprint(
    ...
    if video_budget_bytes <= 0 and videos:
        eprint(
            "ERROR: Assets + audio + overhead exceed target size; no room left for video."
        )
        sys.exit(1)
    ...
    if computed_kbps > MAX_SVT_KBPS:
        eprint(
            ...
            "Final size will undershoot target."
        )
    ...
    eprint(
        f"Video duration: {total_duration/3600:.2f} h; Audio bytes: {total_audio_bytes:,}"
    )
    if videos:
        eprint(
            f"Video budget: {max(0, video_budget_bytes):,} bytes; "
