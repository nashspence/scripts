#!/usr/bin/env python3
import argparse, os, sys, json, time, pathlib, hashlib, shutil
...
def verify_copy_srcsize(src_size: int, dst: str, use_hash: bool, algo: str, src_path_for_hash: Optional[str]) -> bool:
    try:
        ds = os.stat(dst)
...
def copy_atomic_infinite_retry(src: str, dst: str, use_hash: bool, algo: str):
    ...
    while True:
        ...
        try:
            ...
            try: shutil.copystat(src, tmp, follow_symlinks=True)
...
def unique_with_suffix(dest_dir: str, rel_path: str, used: set) -> str:
    ...
    while candidate in used or os.path.exists(os.path.join(dest_dir, candidate)):
        candidate = os.path.join(base_dir, f"{stem}_{k}{ext}") if base_dir else f"{stem}_{k}{ext}"
        k += 1
    used.add(candidate)
    return candidate
...
def main():
    ...
    ap.add_argument("--pattern", default=None, help="Optional glob to filter (e.g., '*.pdf').")

    # Destination + manifest
    ap.add_argument("--dest-dir", default="/out", help="Destination staging directory.")
    ap.add_argument("--manifest-name", default=DEFAULT_MANIFEST, help="Manifest filename under dest dir.")

    # Behavior
    ap.add_argument("--keep-sources", action="store_true", help="Do NOT delete sources after verified copy.")
    ap.add_argument("--hash", default="sha256", help="Hash algorithm (sha256, sha1, blake2b, ...).")
    ap.add_argument("--skip-hash", action="store_true",
                    help="Skip content hashing. Size-only verification (faster, less strict).")
    ap.add_argument("--dry-run", action="store_true", help="List planned actions; make no changes.")
    ...
    if manifest_indicates_completed(manifest):
        eprint("=== stage_generic: manifest indicates job already complete; nothing to do. ===")
    ...
    for root in roots:
        for f in walk_root(root):
            ...
            if os.path.abspath(apath).startswith(os.path.abspath(args.dest_dir) + os.sep):
                continue
    ...
    eprint(f"Delete sources after verified copy: {'YES' if not args.keep_sources else 'NO'}")
    ...
    for (src, root) in file_entries:
        ...
        if prev and "dst" in prev and prev["dst"].startswith(os.path.abspath(args.dest_dir) + os.sep):
    ...
    for (src, dst, key, st) in planned:
        ...
        rec.update({"src":src, "dst":dst, "size":st.st_size, "mtime":int(st.st_mtime)})
        manifest["items"][key] = rec; save_manifest(manifest, manifest_path)

        # Already good?
        if os.path.exists(dst) and verify_copy_srcsize(rec["size"], dst, use_hash=not args.skip_hash, algo=args.hash, src_path_for_hash=src):
            if rec.get("status") != "done":
        ...
        copy_atomic_infinite_retry(src, dst, use_hash=not args.skip_hash, algo=args.hash)
        if not verify_copy_srcsize(rec["size"], dst, use_hash=not args.skip_hash, algo=args.hash, src_path_for_hash=src):
            eprint(f"ERROR: could not verify copy for {src} -> {dst}. Will retry on next run.")
    ...
    if success:
        ...
        eprint(f"All items complete. Manifest marked complete and retained: {manifest_path}")
    else:
