#!/usr/bin/env python3
import argparse, os, sys, subprocess, re, random, time, json, shlex
...
def ffprobe_duration(path: str, timeout_sec: int = 30) -> float:
    ...
    log(f"ffprobe done ({time.time()-t0:.2f}s): {os.path.basename(path)} -> {dur_s or 'N/A'}")
    try:
        return float(dur_s)
    except Exception:
        return 0.0
...
_PATTERNS = [
    re.compile(r"(?<!\d)(\d{4})[._-]?([01]\d)[._-]?([0-3]\d)[ T_-]?([0-2]\d)[.:_-]?([0-5]\d)[.:_-]?([0-5]\d)(?!\d)"),
    re.compile(r"(?<!\d)(\d{2})[._-]?([01]\d)[._-]?([0-3]\d)[ T_-]?([0-2]\d)[.:_-]?([0-5]\d)[.:_-]?([0-5]\d)(?!\d)"),
...
def save_manifest(out_dir: str, m: dict):
    ...
    with open(tmp,"w",encoding="utf-8") as f: json.dump(m, f, indent=2); f.write("\n")
...
def current_sources_sig(src_dir: str) -> list[dict]:
    ...
    for p in files:
        try:
            st = os.stat(p)
            stats.append({"path": os.path.abspath(p), "size": st.st_size, "mtime": int(st.st_mtime)})
...
def main():
    ...
    ap.add_argument("--src-dir", default="/in", help="Directory containing source video files (default: /in).")
    ap.add_argument("--target", type=int, default=int(os.getenv("TARGET", "300")), help="Target minutes total (default 300).")
    ap.add_argument("--min", type=int, default=int(os.getenv("MIN", "6")), help="Minimum clip minutes (default 6).")
    ap.add_argument("--max", type=int, default=int(os.getenv("MAX", "9")), help="Maximum clip minutes (default 9).")
    ...
    ap.add_argument("--debug-cmds", action="store_true",
    ...
    if not os.path.isdir(args.src_dir):
        eprint(f"[autoedit] ERROR: --src-dir must be an existing directory (got: {args.src_dir})")
        sys.exit(1)
    ...
    if m and m.get("final", {}).get("status") == "done":
        ...
        if outp and os.path.exists(outp) and ffprobe_duration(outp) > 0:
            log(f"Manifest indicates job already completed successfully at {finished_at}. Output: {outp}. Returning.")
            print(outp)
            return
        else:
            log("Manifest says 'done' but final output is missing or invalid; proceeding with rebuild.")
    ...
    log("Start")
    log(f"Source dir: {m['sources']['src_dir']}  "
    ...
    if not files:
        eprint("[autoedit] ERROR: no video files found in source directory"); sys.exit(1)

    log(f"Probing durations for {len(files)} file(s)…")
    durations = [ffprobe_duration(p) for p in files]
    if not any(d>0 for d in durations):
        eprint("[autoedit] ERROR: cannot read durations"); sys.exit(1)
    combined = sum(durations)

    # Build or load the plan
    if not m.get("plan"):
        target_min = args.target if combined >= args.target*60 else int(combined // 60)
        ...
        if slot_count < len(files):
            log(f"WARNING: slot_count ({slot_count}) < number of files ({len(files)}). "
                "Some files may receive 0 quota.")

        q = quotas(durations, slot_count, args.min)
        if sum(q) != slot_count:
            log(f"WARNING: quota sum {sum(q)} != slot_count {slot_count}; continuing with computed q.")
        ...
        plan = {
            ...
            "files": [{"path": files[i], "duration": durations[i], "quota": q[i], "base_epoch": base_epochs[i]} for i in range(len(files))]
        }
        clips = {}
        idx = 1
        ...
        for i, fi in enumerate(plan["files"]):
            qi = fi["quota"]
            if qi == 0: continue
            d = fi["duration"]; part = d/qi if qi>0 else 0.0
        ...
        log(f"Plan: target={target_min} min, slots={len_slots_min} (count={len(len_slots_min)}), files={len(files)}")
        for i, fi in enumerate(plan["files"]):
            log(f"  file[{i}] quota={fi['quota']} duration={fi['duration']:.1f}s name={os.path.basename(fi['path'])}")
    ...
    done = sum(1 for c in m["clips"].values()
               if c["status"] == "done" and os.path.exists(c["out"]) and ffprobe_duration(c["out"])>0)
    total = len(m["clips"])
    ...
    for k in sorted(m["clips"], key=lambda x: int(x)):
        clip = m["clips"][k]
        out_clip = clip["out"]
        if clip["status"] == "done" and os.path.exists(out_clip) and ffprobe_duration(out_clip) > 0:
            continue

        src = clip["src"]; start = clip["start"]; L = clip["length"]; epoch_int = clip["epoch"]
        ...
        cmd = [
            ...
            "-ss", f"{start:.6f}", "-t", f"{L:.6f}", "-i", src,
            "-fps_mode","passthrough",
            "-map","0:v:0","-map","0:s?","-map","0:d?","-map","0:t?",
            "-filter_complex", fcomplex,
        ] + map_seq + [
            "-c:v","libsvtav1",
            "-preset", str(m["plan"]["svt_preset"]),
            "-crf", str(m["plan"]["svt_crf"]),
            "-g", str(m["plan"]["svt_gop"]),
            "-svtav1-params", f"keyint={m['plan']['svt_gop']}:irefresh-type=2:scd=1",
            "-c:a","libopus","-b:a", m["plan"]["opus_br"],
        ...
        log(f"clip {int(k):03d} START ← {os.path.basename(src)} @ {start:.2f}s for {L:.0f}s → {os.path.basename(out_clip)}")
        if args.debug_cmds:
            log("CMD: " + " ".join(shlex.quote(x) for x in cmd))
        t0 = time.time()
        r = subprocess.run(cmd)
        size_now = os.path.getsize(out_clip) if os.path.exists(out_clip) else 0
        log(f"clip {int(k):03d} DONE rc={r.returncode} in {time.time()-t0:.1f}s size={size_now} bytes")
    ...
    out_path = os.path.join(args.autoedit_dir, f"{datetime.now().strftime('%Y-%m-%d')} auto-edit.mkv")
    m["final"]["out_path"] = out_path
    save_manifest(args.autoedit_dir, m)

    all_done = all(c["status"] == "done" and os.path.exists(c["out"]) for c in m["clips"].values())
    if not all_done:
    ...
    cmd_final = [
        ...
        "-c","copy","-cues_to_front","1","-reserve_index_space","200k",
        out_path
    ]
